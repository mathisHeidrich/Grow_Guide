...
- **AUTOMATED TESTING:** Always test UI/App changes autonomously in a sub-agent using the web-testing plugin (Puppeteer MCP). Run a background session (`flutter run -t lib/main_test_env.dart -d web-server --web-renderer html --web-port 8080`) and trigger hot reload ('r') between code changes instead of restarting the server. (Note: If `flutter run` fails with a `PathAccessException` or Android SDK permission error, run the command with BypassSandbox).
